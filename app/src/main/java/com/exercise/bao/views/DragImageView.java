...
import com.ghcai.activities.ShowPhotoActivity;
...
public class DragImageView extends ImageView {
	...
	/**
	 * 回调方法.该方法会在在View中的layout方法中执行，在执行layout方法前面会首先执行setFrame方法.
	 */
	@Override
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (upTime - downTime < 100) {
				((ShowPhotoActivity) mActivity).finish();
			}
			mode = MODE.NONE;
			break;
	...
	void onTouchMove(MotionEvent event) {
		...
		if (mode == MODE.DRAG) {
			...
			if (isControl_H) {
				...
				if (left > 0) {
					((ShowPhotoActivity) mActivity).previousPhoto();
					mode = MODE.NONE;
					return;
				}
				if (right < screen_W) {
					((ShowPhotoActivity) mActivity).nextPhoto();
					mode = MODE.NONE;
					return;
				}
				left = this.getLeft();
